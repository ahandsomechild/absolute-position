<template>
  <div class="sign-index">
    <div class="top center">
      ...
      <span class="text-lg">这是一个合同</span>
      <div class="right pull-right">
        <span style="cursor: pointer" @click="save_draft">保存草稿</span>&nbsp;
        <Button type="primary" class="button-flat" @click="save_img">下一步</Button>
      </div>
    </div>
    <div class="content">
        <Row :gutter="16">
            <Col :span="4" class="left border-right">
              <div class="title border-bottom">
                合同缩略图
              </div>
              <div class="preview">
                <div style="border:1px solid #ddd;height:250px;width:80%;margin:10px auto;">
                  
                </div>
              </div>
            </Col>
            <Col :span="16" class="middle border-left border-right">
                <div class="container" id="sign-container" @scroll="cscroll()" ref="container">
                  <div class="item"
                    ...
                    <div draggable="true" style="cursor: move" class="sign-position center" v-for="(st, ind) in signs"
                        @dragstart='drag(ind, $event)'  @drag="draging(ind, $event)"
                        :style="{ left: st.limitx * 100 +'%', top: st.limity  * 100 +'%'}" ref="sp">
                      <i class="el-icon-circle-close-outline pull-right text-lg" title="删除签名" @click="deleteSign(st,ind)"></i>
                      <span class="center ellipsis">糖糖糖签署区域</span>
                    </div>
                  </div>
                </div>
            </Col>
            <Col :span="4" class="right border-left">
                <div class="title border-bottom">
                ...
                <div class="container" >
                    ...
                    <div class="ccenter">
                      <div class="item" draggable="true" @dragstart="rightDragStart(signers[0], 0,$event)">
                        ...
                        <div class="number active">
                          <span title="点击添加签署位置"  @click="signclc(signers[0], 0)" >
                            <!---->
                            签署位置N处</span>
                        </div>
                      </div>
                  ...
                  <div class="receiver">
                    ...
                    <div class="ccenter">
                      <div class="item" draggable="true" @dragstart="rightDragStart(item, index,$event)">
                        <div class="name">
                          哈哈哈
                        </div>
                        <div class="number">
                          <span title="点击添加签署位置" @click="signclc(item, index)">签署位置N处</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  export default {
    data() {
      return {
        ...
        containerH: 841.92, //中间文档高度
        imgH: 978,       //中间图片的高度
        imgW: 649,       //中间图片的宽
        signH: 97.8,         //签名的高度(A4纸的10%)
        signW: 162.25,         //签名的宽度(A4纸的25%)
      }
    },
    computed: {
      /**
    ...
    methods: {
      ...
      dragend(index, e) {
        ...
        } else this.draged.limitx = xlen + this.draged.limitx
        ...
        } else this.draged.limity = ylen + this.draged.limity
